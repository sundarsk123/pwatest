...
import banner from '@/Assets/Product/attachments/Product.jpg'
...
const ProductIntro = ({ data }) => {
    ...
    return (
        <div className='relative'>
            <div>
                <Image
                    src={data?.acf?.products_banner_image.url}
                    width={1521}
                    height={547}
                    alt={data?.acf?.products_banner_image.alt}
            ...
            <div className='absolute bottom-0 left-0 right-0 px-[20px] md:px-[3.75rem] pb-[1.25rem] md:pb-[2.25rem]'>
                <h1 ref={headingRef} className='text-white text-[1.8rem] md:text-[2.4rem] lg:text-[4rem] pt-1.5 block opacity-0'>{data?.acf?.products_title}</h1>
            </div>
        </div>
    )
}
